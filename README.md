...
**General Guidelines:**
- **Python Standard Library only.** No pip installs, no external dependencies. Just the built-in stuff (`os`, `sys`, `math`, `json`, `re`, etc).
- **Keep it focused.** Do one thing and do it well. Don't try to bundle a bunch of unrelated functionality into a single thing.
- **Make it actually useful.** Think about whether people will actually use this or if it's just niche. We're not against niche, but it should solve a real problem.
- **Test it properly.** Make sure it doesn't break anything else and runs without errors.
- **Document what you're adding.** Help text, docstrings, usage examples — whatever it takes for someone to understand what this does.
